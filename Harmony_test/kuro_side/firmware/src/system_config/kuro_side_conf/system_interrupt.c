...
{           
    ...
    appData.state = APP_STATE_INT1_EVENT;
}
...
void __ISR(_EXTERNAL_2_VECTOR, IPL1AUTO) _IntHandlerExternalInterruptInstance1(void) {
    ...
    appData.state = APP_STATE_INT2_EVENT;
}
...
void __ISR(_EXTERNAL_3_VECTOR, IPL1AUTO) _IntHandlerExternalInterruptInstance2(void) {
    ...
    appData.state = APP_STATE_INT3_EVENT;
}
...
void __ISR(_TIMER_5_VECTOR, ipl1AUTO) _IntHandlerDrvTmrInstance1(void)
{

    PLIB_INT_SourceFlagClear(INT_ID_0,INT_SOURCE_TIMER_5);

        appData.state = APP_STATE_TIMER_EVENT;
}

void __ISR(_UART_1_VECTOR, ipl1AUTO) _IntHandlerDrvUsartInstance0(void) {
    /* TODO: Add code to process interrupt here */
    while (DRV_USART0_ReceiverBufferIsEmpty());
    appData.mensajeRx.cmd = DRV_USART0_ReadByte();
    while (DRV_USART0_ReceiverBufferIsEmpty());
    appData.mensajeRx.nDatos = DRV_USART0_ReadByte();
    int i = 0;
    for (i = 0; i < appData.mensajeRx.nDatos; i++) {
        while (DRV_USART0_ReceiverBufferIsEmpty());
        appData.mensajeRx.Datos[i] = DRV_USART0_ReadByte();
    }
    while (DRV_USART0_ReceiverBufferIsEmpty());
    appData.mensajeRx.cksum = DRV_USART0_ReadByte();
